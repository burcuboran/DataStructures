...
public class FractalApplication extends Application {
	...
	public void start(Stage stage) {
		fractals = new Fractal[5];
		fractals[0] = new Koch(300);
		fractals[1] = new Mountain(40, new Point(70,400), new Point(200,90), new Point(450,400));
		fractals[2] = new Tree(100);
		fractals[3] = new Sierpinski(300);
		fractals[4] = new Julia(300);
			...
			@Override protected Void call() throws Exception {
				...
				Platform.runLater(new Runnable() {
					@Override public void run() {
						labelStatus.setText("Drawing the fractal");
				...
				actFractal.draw(tg,gc);
				long timeEnd = System.currentTimeMillis();
				// modifying the javafx scene graph must be done from the javafx application thread
				Platform.runLater(new Runnable() {
					@Override public void run() {
						labelStatus.setText("Complete, it toke " + (timeEnd - timeStart) + " miliseconds.");
...
//					System.err.println(e.getMessage());
	...
	/**
	 * A label to show the line count. An animation thread is created to update the label 10 times 
	 * per second.
	 * @author pera
	 *
	 */
			...
			Timeline timeline = new Timeline(
					new KeyFrame(Duration.seconds(0),
						...
						@Override public void handle(ActionEvent actionEvent) {
							if(tg != null){
								setText("Total lines: " + tg.getLineCount());
							}
						}
					}
							),
